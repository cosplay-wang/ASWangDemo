...
/**
 * Created by zhiwei.wang on 2017/3/15.
 */

public class ListviewItemViewGroup extends ViewGroup {
    private static final String TAG = "zxt/ListviewItemViewGroup";
    ...
    //存储的是当前正在展开的View
    private static ListviewItemViewGroup mViewCache;
    ...
    public ListviewItemViewGroup(Context context) {
        this(context, null);
    }

    public ListviewItemViewGroup(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ListviewItemViewGroup(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * @param ios
     */
    public ListviewItemViewGroup setIos(boolean ios) {
        isIos = ios;
        return this;
    }
     ...
     * @return
     */
    public ListviewItemViewGroup setLeftSwipe(boolean leftSwipe) {
     ...
     * @return
     */
    public static ListviewItemViewGroup getViewCache() {
        return mViewCache;
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //左滑右滑的开关,默认左滑打开菜单
        isLeftSwipe = true;
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.ListviewItemViewGroup, defStyleAttr, 0);
        int count = ta.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = ta.getIndex(i);
            //如果引用成AndroidLib 资源都不是常量，无法使用switch case
            if (attr == R.styleable.ListviewItemViewGroup_swipeEnable) {
                isSwipeEnable = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.ListviewItemViewGroup_ios) {
                isIos = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.ListviewItemViewGroup_leftSwipe) {
...
/*                    if (!mScroller.isFinished()) {
    ...
    public void smoothExpand() {
        ...
        invalidate();*/
        //展开就加入ViewCache：
        mViewCache = ListviewItemViewGroup.this;
